...
ACME-0188 extends the existing native OpenAI Responses adapter with ordered user image input. Provider-neutral user `text` and `image` content now map to ordered Responses `input_text` and `input_image` parts; image `dataRef` is carried as `image_url`. Empty refs fail before transport, and assistant-image history remains unsupported. Release 0.1.5 carries the change through `@acme-engine/adapter-model-openai@0.1.5 -> @acme-engine/model-runtime@0.1.5 -> acme-engine@0.1.5`. A clean registry-only consumer proved the installed chain emits `https://api.openai.com/v1/responses` with mixed text/image input. This restores the native Responses route needed by embedded A008 without changing Chat Completions behavior.
...
`acme-engine@0.1.5` is the current installable npm facade over `@acme-engine/model-runtime@0.1.5`; the native OpenAI Responses path resolves `@acme-engine/adapter-model-openai@0.1.5`, while the Chat Completions dependency path remains `@acme-engine/adapter-model-chat-completions@0.1.4`. ACME-0184 repairs the initial 0.1.0 registry publication, whose manifests incorrectly preserved `workspace:*` internal dependencies because the workspace directories were published directly. Version 0.1.1 established the pnpm-packed tarball publication baseline. ACME-0186 publishes the bounded timeout-classification fix as 0.1.3: after a successful HTTP response start, an explicit transport timeout remains ACME `TIMEOUT`, while non-timeout stream interruption remains `MODEL_INVALID_RESPONSE`. A clean registry-only consumer installed `acme-engine@0.1.3` and reproduced the timeout result (`ACME_0186_REGISTRY_CONSUMER_OK`). ACME-0187 then adds an explicit Chat Completions profile option for the output-token wire parameter: `max_tokens` remains the backward-compatible default, while callers such as A008 may select `max_completion_tokens` for OpenAI chat models. Release 0.1.4 carries that bounded change. A clean registry-only 0.1.4 consumer proved `max_completion_tokens` is emitted with no `max_tokens`, and an A008 detached proof on registry 0.1.4 passed its embedded OpenAI wire regression. ACME-0188 then publishes 0.1.5 with native OpenAI Responses image parity; a clean registry-only 0.1.5 consumer proved the native `openAi` route emits ordered `input_text` + `input_image` to `/v1/responses`. Version 0.1.0 remains immutable registry history and must not be used. The workspace root remains `@acme-engine/workspace`, `private: true`.


## Explicit non-stream model execution intent

ACME-0185 adds optional `ModelRequest.stream` to the public model contract and makes `ModelExecutionEngine` use `ModelGateway.generate()` when the caller explicitly sets `stream: false`, even when the gateway also exposes `stream()`. Requests that do not explicitly disable streaming keep the existing streaming path. This fixes embedded consumers whose provider correctly returns ordinary JSON for `stream:false`; ACME no longer feeds that JSON to an SSE parser and misclassifies it as a truncated stream. Release 0.1.2 carried that change; the current public release is `0.1.5` after ACME-0188. Current full verification passes 1086/1086 unit and 86/86 conformance tests in addition to build, typecheck, lint, boundaries, format and docs checks.
