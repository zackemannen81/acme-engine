...
`acme-engine@0.1.3` is the current installable npm facade over `@acme-engine/model-runtime@0.1.3`; the Chat Completions dependency path also resolves `@acme-engine/adapter-model-chat-completions@0.1.3`. ACME-0184 repairs the initial 0.1.0 registry publication, whose manifests incorrectly preserved `workspace:*` internal dependencies because the workspace directories were published directly. Version 0.1.1 established the pnpm-packed tarball publication baseline. ACME-0186 publishes the bounded timeout-classification fix as 0.1.3: after a successful HTTP response start, an explicit transport timeout remains ACME `TIMEOUT`, while non-timeout stream interruption remains `MODEL_INVALID_RESPONSE`. A clean registry-only consumer installed `acme-engine@0.1.3` and reproduced the timeout result (`ACME_0186_REGISTRY_CONSUMER_OK`). Version 0.1.0 remains immutable registry history and must not be used. The workspace root remains `@acme-engine/workspace`, `private: true`.


## Explicit non-stream model execution intent

ACME-0185 adds optional `ModelRequest.stream` to the public model contract and makes `ModelExecutionEngine` use `ModelGateway.generate()` when the caller explicitly sets `stream: false`, even when the gateway also exposes `stream()`. Requests that do not explicitly disable streaming keep the existing streaming path. This fixes embedded consumers whose provider correctly returns ordinary JSON for `stream:false`; ACME no longer feeds that JSON to an SSE parser and misclassifies it as a truncated stream. Release 0.1.2 carried that change; the current public release is `0.1.3` after ACME-0186. Current full verification passes 1080/1080 unit and 86/86 conformance tests in addition to build, typecheck, lint, boundaries, format and docs checks.
