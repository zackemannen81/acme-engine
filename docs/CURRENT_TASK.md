...
- [x] `pnpm build`
- [x] `git diff --check`
- [~] Recorded run against the live Supabase case: extract one instance, review
...
- [~] Recorded run against the live Supabase case — **done except the live
      extraction**. See Verification: the charter forbids live spend and the
      gate requires an extraction, and that contradiction is the operator's to
      resolve, not mine to absorb.
...
- `pnpm test`: unit 892/892 (up from 871), conformance 78, integration 70,
  scenario 26. 21 new tests: 13 over the fold, 8 over the routes.
...
The frozen gate says "extract one instance"; the frozen Out of Scope says
"Live model spend". Both cannot hold, and the contradiction is mine — I wrote
it into the charter. It is recorded rather than resolved by quietly picking
whichever half suits the result.

The plan route states the cost without paying it: the Hussein instance
`instance-part-000381` plans **2** bounded model calls, 2 windows, 0 already
committed. Everything the extraction would add to this task is the
*model-produced* half of a review loop that is already proven end to end on
reviewer-authored evidence over the real binder.

Deciding to spend is the operator's, so this task pauses here rather than
either skipping a frozen gate or spending without being asked.
...
- Current state: `In Progress`, paused at a clean boundary. Every deliverable
  is implemented, tested and recorded; one frozen verification gate is unmet.
- Next recommended step: the operator decides whether to spend the planned 2
  model calls on `instance-part-000381`. If yes, run the extraction, review
  the model-produced occurrences and close the task. If no, the charter is
  amended or the task is superseded — not silently completed.
- Blockers: the charter contradicts itself on live spend. See Verification.
- Child tasks: none.
- Resume condition: an operator decision on the 2-call extraction.
- Open questions: none beyond that decision.
