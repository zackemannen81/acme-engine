...
- Focused Chat Completions/model-runtime regression: 28/28 passed.
- Full required gates passed: docs check, format check, lint, dependency boundaries, typecheck and forced build.
- Full unit suite: 161 files, 1080/1080 tests passed.
- Conformance suite: 13 files, 86/86 tests passed.
- Published registry path is `acme-engine@0.1.3` -> `@acme-engine/model-runtime@0.1.3` -> `@acme-engine/adapter-model-chat-completions@0.1.3`.
- Clean registry-only consumer reproduced HTTP response-start followed by transport timeout as terminal ACME `TIMEOUT` (`ACME_0186_REGISTRY_CONSUMER_OK`).
- Non-timeout post-response-start interruption remains covered as `MODEL_INVALID_RESPONSE`.
- The temporary A008 live-runtime helper was removed from the final branch diff; it remains preserved in checkpoint history.
