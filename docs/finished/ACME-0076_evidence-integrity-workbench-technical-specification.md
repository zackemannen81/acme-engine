...
- [x] Verify that every authority level and invariant the specification
  proposes traces to ADR-0028, ADR-0029, the normative product definition or an
  explicitly identified decision still requiring an ADR.
- [x] Verify that every corpus case the specification requires is described
...
- [x] Verify that the specification requires every accepted observation and
...
- [x] Verify that the specification defines the negative fixtures and refusal
...
- [x] Verify that the specification defines both product acceptance tests as
...
- [x] Verify that the specification's staleness model is derived from recorded
  revisions and citation identity only, with no model-scored or inferred
  relevance ranking anywhere in it.
- [x] Review the specification's package boundaries against ACME's dependency
  direction and core vocabulary guardrails.
- [x] Review every Mermaid diagram in the specification by hand, and record
...
- [x] Inventory accepted Evidence meanings and deferred sandbox concepts.
- [x] Design the synthetic matter, corpus matrix and annotation protocol.
- [x] Define minimal domain contracts, identity and ownership.
- [x] Define versioned views, their primary/secondary classification and
  human-review records.
- [x] Define the primary entry screen's job and the Reviewed Evidence
  Assessment outcome object with its deterministic export.
- [x] Define proof matrix, thresholds, denominators and prohibited-output
  fixtures.
- [x] Define dependency boundaries and the two required ADRs.
- [x] Produce separately activatable implementation slices as reviewer
  capabilities.
- [x] Write the normative technical specification.
- [x] Synchronize relevant governing docs and indexes.
- [x] Verify, journal, archive and restore the next real task state.
...
- 2026-08-11: Charter frozen at `Ready`; status then advanced to
  `In Progress`. No charter semantics changed.

## Verification

- [x] Draft review completed before transition to `Ready`.
- [x] Corpus and annotation consistency review.
- [x] Authority/invariant traceability matrix review.
- [x] Architecture and dependency-boundary review.
- [x] Product separation review: Primary Product Rule, view classification and
  both acceptance tests.
- [x] `pnpm docs:check`.
- [x] `git diff --check`.
- [x] Manual Mermaid review, recorded together with the skipped automated
  Mermaid validation and its reason.
- [x] Document any further skipped check and exact reason.

Verification results:

- Corpus/annotation consistency: confirmed seven inventory rows, ten sealed
  observation rows, eight sealed relation rows, three open questions and two
  assessment versions with internally consistent standings and scenario order.
- Authority and architecture: every L0–L5 rule and persistent invariant maps to
  an accepted ADR, the product definition or this frozen specification; all
  planned dependencies follow apps → adapters/contracts → module → core.
- Product separation: nine primary and two secondary view contracts are
  classified; both executable acceptance tests and their exact vocabulary
  rules are defined.
- `corepack pnpm docs:check`: passed; final post-archive pass checked 157
  Markdown files for internal links and balanced fences.
- `git diff --check`: passed with the pre-existing untracked
  `package-lock.json` preserved and untouched.
- The specification's single Mermaid flow was reviewed manually: node ids are
  unique, every referenced node exists, the direction is acyclic and the
  candidate-refusal branch terminates at excluded L5 authority.
- Automated Mermaid validation was deliberately skipped for this exact reason:
  the repository has no Mermaid validator, and `tooling/docs/check-docs.mjs`
  verifies internal links and balanced fences only.
- Code gates were not run because ACME-0076 changes documentation and accepted
  architecture only; it adds no code, dependency, schema migration or runtime
  behavior.
...
- [x] `docs/adr/README.md`, plus every ADR the specification identifies as
  required before implementation

## Handoff and Follow-ups

- Current state: Complete. The normative specification and ADR-0030/0031 are
  accepted, governing docs are synchronized and no product code was added.
- Next recommended step: explicitly activate slice 0, Corpus and contracts
  foundation, as a new frozen task before authoring fixtures or packages.
- Blockers: none for planning. Implementation remains unactivated and every
  slice requires its own approved charter.
- Child tasks: none.
- Resume condition: not applicable.
- Open questions: none inside ACME-0076. PostgreSQL schema/transactions,
  identity/authorization, object-storage consistency and any non-synthetic
  path remain explicit later ADR gates.
