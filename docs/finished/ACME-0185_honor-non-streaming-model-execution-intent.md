...
- [x] Focused ModelExecutionEngine generate-vs-stream regression.
- [x] Chat Completions explicit non-stream JSON regression.
- [x] Existing streaming engine/adapter regression.
- [x] Public closure build/typecheck/tests.
- [x] Version/lockfile consistency at 0.1.2.
- [x] Tarball manifests contain no `workspace:*`.
- [x] Clean tarball consumer proof.
- [x] npm publication from verified tarballs in dependency order.
- [x] Registry manifest verification.
- [x] Clean registry-only consumer import/runtime proof.
- [x] `pnpm docs:check` and `git diff --check`.

## Checklist

- [x] Add failing regression that proves explicit `stream:false` must use `generate()`.
- [x] Make the minimal engine selection fix.
- [x] Run focused and public-closure verification.
- [x] Update version/package-lock metadata to 0.1.2.
- [x] Pack and inspect all public tarballs.
- [x] Prove tarballs in a clean external consumer.
- [x] Publish verified tarballs and inspect registry manifests.
- [x] Prove registry-only install/import.
- [x] Update CURRENT_STATUS, SYSTEMDOC and JOURNAL.
- [x] Archive the completed task and restore CURRENT_TASK template.
...
- Red/green proof: explicit `stream:false` initially failed validation, then passed after the additive ModelRequest contract and engine selection fix.
- Focused core + model-runtime: 14/14 PASS.
- Full `pnpm build`: PASS.
- Full `pnpm typecheck`: PASS.
- Full unit suite: 161/161 files, 1078/1078 tests PASS.
- Conformance: 86/86 PASS.
- Seven 0.1.2 tarballs packed; `PACK_MANIFESTS_OK` proved exact internal 0.1.2 dependencies and zero packed `workspace:*`.
- Clean tarball consumer: `ACME_0185_TARBALL_CONSUMER_OK tarball-ok send=1 stream=0`.
- All seven verified tarballs published in dependency order. Registry manifests expose concrete 0.1.2 internal dependencies.
- Clean registry-only consumer: `ACME_0185_REGISTRY_CONSUMER_OK registry-ok send=1 stream=0`; installed tree resolves `acme-engine@0.1.2 -> @acme-engine/model-runtime@0.1.2 -> @acme-engine/core@0.1.2`.
