...
- [ ] Exact isolated six-call live journey once — stopped after the first
  successful job on a source-view field-name defect
- [x] Content-free job/domain/persistence assertions through the stop point
...
- [x] Run exact isolated journey once; one of six calls consumed, no retry.
...
- The first D1 observation job completed and committed with one call: stop,
  66,818 input + 491 output = 67,309 total tokens; eight output `/4`
  candidates, eight valid unique supplied segments and eight runtime-derived
  observations. The first reviewer POST then failed before any further job:
  the harness read `observationId` from the primary source view, whose contract
  exposes `observationVersionId`, so it sent an undefined target and received
  a fail-closed 400. Zero review decisions, relations or assessments wrote.
- No retry occurred. Exact containers, network, two temporary sources and four
  key/credential files were removed. Both parent PDF hashes are unchanged and
  `.env.local` remains present and ignored.
...
- [x] Record exact job/domain/persistence outcome without content.
...
- [x] `docs/SYSTEMDOC.md`
- [x] `docs/JOURNAL.md`
- [x] completion plan and Slice 9 checklist; `docs/FILESTRUCTURE.md` not changed

## Handoff and Follow-ups

- Current state: one successful D1 observation call committed; journey stopped
  on a deterministic harness field-name defect and all disposable state is gone.
- Next recommended step: correct `observationVersionId` in a separate offline
  task, verify, then freeze a new one-shot live journey.
- Blockers: this task's no-retry run is consumed.
- Child tasks: none.
