...
- [x] `pnpm build`
- [x] `pnpm test:live` reaching a success response
- [x] `git diff --check`
...
- [x] Read the required documents and ACME-0028's recorded evidence.
...
- [x] Implement the lowering and its determinism test.
- [x] Implement the local preflight refusal.
- [x] Prove the lowered form against the provider, including nested `anyOf`.
- [x] Implement un-lowering per the decision taken (replaced by `.nullish()`;
      no adapter un-lowering).
- [x] Record `providerWireSchemaHash` alongside the canonical hash.
- [x] Reach a live success response and confirm or correct the fixtures.
- [x] Verify hash-only retention and the `unavailable` replay verdict.
- [x] Run every frozen verification gate and record evidence and spend.
- [x] Update the long-lived documentation, ADR-0014 and the journal.
- [x] Archive ACME-0029 and restore or repopulate `docs/CURRENT_TASK.md`.
...
- [x] Prove the lowering is deterministic byte-for-byte.
- [x] Prove an unlowerable schema is refused locally with no network call.
- [x] Prove nested `anyOf` is accepted by the provider, empirically.
...
- [x] Prove the live execution commits, persists no payload and replays as
      `unavailable`.
- [x] Record exact test counts for every gate, and actual spend.
- [x] Document skipped checks and reasons.

### Offline evidence (2026-08-01, post-lowering)

- `lowerStrictStructuredOutputSchema` is pure, deterministic under
  `canonicalJson`, and covered by `packages/adapter-model-openai/test/schema-lower.test.ts`.
- Unlowerable schemas raise `UNSUPPORTED_CAPABILITY` with `details.construct`
  and `details.path`; gateway preflight leaves `transport.sent` empty.
- Both reference-domain output schemas lower without refusal: narrative
  `oneOf` → nested `anyOf` with distinct `type` consts; research optionals
  become required-and-nullable. No `oneOf` remains on the wire form.
- `providerWireSchemaHash` (`acme-provider-wire-schema-hash-1`) is returned
  from `buildResponsesBody` and recorded on success/incomplete response
  `metadata` without touching canonical `acme-model-request-hash-1`.
- Final offline gates: `pnpm docs:check` (68+ Markdown files),
  `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm boundaries`,
  `pnpm test:unit`, `pnpm test:conformance` (46), `pnpm test:integration` (13),
  `pnpm test:scenario` (19), `pnpm build`, `git diff --check`.

### Live evidence (2026-08-01)

1. Default model `gpt-5.6-terra`: HTTP 400
   `Unsupported parameter: 'temperature' is not supported with this model.`
   Schema was accepted (not `invalid_json_schema`). Free rejection after
   schema validation.
2. `ACME_LIVE_MODEL=gpt-4.1-mini`, research.observe-evidence via
   `pnpm test:live`: HTTP 200, engine `committed`,
   `OpenAiResponseSchema` accepted the real body, call retained
   `responseHash` only (`response` undefined), `replayVerify` →
   `unavailable`. Usage ≈ 463 in + 75 out = 538 total tokens.
3. Narrative.observe-document probe (nested `anyOf` on wire, no `oneOf`):
   HTTP 200, engine `committed`. Usage ≈ 509 in + 165 out = 674 total tokens.

Skipped: none of the frozen offline gates. Remote GitHub Actions not run
(no push authorized). Currency spend is not machine-metered; token totals
above are the provider-reported usage for the two successful calls.
...
- [x] ADR-0014's limitation section, once the success-path fixtures are
      confirmed or corrected.
- [x] A new ADR if the lowering, its refusal contract or the second schema
      identity constrains future providers, which it probably does.
      → ADR-0015.

## Handoff and Follow-ups

- Current state: Status `Complete`. Lowering, preflight, live success (research
  + narrative nested anyOf), docs and ADR-0015 are done.
- Next recommended step: Archive this task and leave `CURRENT_TASK` empty or
  the next explicitly approved charter. Optional backlog: model-parameter
  capability gating for temperature on reasoning models.
- Blockers: None.
- Child tasks: None.
