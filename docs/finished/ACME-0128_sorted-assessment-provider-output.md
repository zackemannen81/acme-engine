...
- [x] Focused contract/hash/replay/registry/composition tests: 17/17.
- [x] Historical `@1.0.0`, historical `@1.1.0` and active `@1.2.0` hashes
  recorded above and asserted byte-exact.
- [x] Typecheck, lint, boundaries and build passed.
- [x] Full default suites passed: 753 unit, 78 conformance, 62 integration
  and 26 scenario tests.
- [x] Fresh disposable PostgreSQL 16 suite passed 36/36; container removed.
- [x] Format, docs and `git diff --check` passed.
- [x] No credential load or provider call occurred.
...
- Current state: active `@1.2.0` and both historical contracts are registered,
  hash-pinned and green across all canonical gates; no provider call occurred.
- Next recommended step: freeze the separately bounded two-source live journey.
- Blockers: none.
- Child tasks: none.
