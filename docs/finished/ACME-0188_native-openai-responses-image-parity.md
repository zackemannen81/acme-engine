...
- [x] Focused OpenAI adapter request/gateway tests.
- [x] Model-runtime native OpenAI image route regression.
- [x] Adapter/model-runtime typecheck + build.
- [x] Full unit and conformance suites.
- [x] Format/lint/boundaries/docs/diff checks.
- [x] Packed-manifest inspection.
- [x] Registry-only 0.1.5 consumer proof after publication.
...
- [x] Bump/publish public closure 0.1.5.
- [x] Verify clean registry consumer.
- [x] Update docs, archive task and restore template.
...
- Full static gates passed: format, lint, boundaries, docs-check, typecheck, build and `git diff --check`.
- Full unit suite: 161/161 files, 1086/1086 tests passed.
- Full conformance suite: 13/13 files, 86/86 tests passed.
- Packed 0.1.5 manifests contained concrete registry dependency versions and no `workspace:*` leakage.
- Published `@acme-engine/adapter-model-openai@0.1.5`, `@acme-engine/model-runtime@0.1.5` and `acme-engine@0.1.5`.
- Fresh registry-only consumer installed exactly `acme-engine@0.1.5 -> @acme-engine/model-runtime@0.1.5 -> @acme-engine/adapter-model-openai@0.1.5`.
- Registry-only runtime proof returned `ACME_0188_REGISTRY_CONSUMER_OK` and emitted `https://api.openai.com/v1/responses` with ordered `input_text` + `input_image`, `max_output_tokens: 321` and `reasoning.effort: "none"`.
...
- [x] `docs/SYSTEMDOC.md`
- [x] `docs/JOURNAL.md`
- [x] `docs/FILESTRUCTURE.md` not required; no structure change

## Handoff and Follow-ups

- Current state: complete and published as 0.1.5.
- Next recommended step: A008 should consume 0.1.5 and move the OpenAI/Luna embedded profile from generic compatible Chat Completions back to native `openAi` Responses in a separate A008 task.
- Blockers: none.
- Child tasks: none.
