...
- [x] Run every verification gate and record exact results.
- [x] Update long-lived documentation.
...
- Final verification found an invalid committed `OPT_IN=1` assignment in the
  opt-in live test. It was removed: this both restores TypeScript validity and
  preserves the repository rule that live provider calls require explicit
  environment opt-in. The user's model/output-token changes were untouched.
- The full typecheck exposed a nominal-type leak from ScenarioRunner's public
  dependency on the concrete `QualityEvaluationHarness` class. The runner now
  accepts only the structural `run`/`runWith` surface it uses, keeping
  composition substitutable and source/dist declarations compatible.
...
- [x] Unit tests cover schemas, identity goldens, immutability, registry,
      deterministic evaluation and every recorded-external mismatch.
- [x] Store conformance covers append-only idempotency, collision refusal,
      filtering/order and detached reads.
- [x] Scenario tests cover v1 compatibility, v2 parse/refusal, failed quality
      as a successful evaluate step and explicit verdict assertion failure.
- [x] A real offline execution feeds both evaluator kinds with byte-identical
      stored results and no external effect.
- [x] Full baseline gates pass; no required check was skipped. Results:
      docs 121 Markdown files; unit 560/560 in 64 files; conformance 61/61 in
      8 files; integration 55/55 in 9 files; scenario 24/24 in 5 files;
      formatting, lint, typecheck, boundaries and build all pass.
...
- [x] New ADR
- [x] `docs/backlog/driver-error-classification.md` clarified without changing
      its proposed behavior

## Handoff and Follow-ups

- Current state: complete and ready for archival.
- Next recommended step: activate the explicitly approved documentation and
  presentation task as ACME-0055.
- Blockers: none.
- Child tasks: none.
