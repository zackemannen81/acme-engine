...
- [ ] Exact Stage A one-call live gate — one non-canonical timestamp failed
...
- [x] Run exact isolated acceptance once; schema validation failed closed.
...
- Sole call returned stop, 66,775 input + 1,533 output = 68,308 tokens and
  eight output `/4` candidates. All eight segment IDs were valid, supplied,
  existing and unique. Seven temporal bounds were `unknown`; one `exact.at`
  was a 16-character date/time containing `T` but no seconds, offset or `Z`.
  Strict schema validation emitted one `MODEL_RESPONSE_SCHEMA` issue. One
  encrypted model call succeeded; zero documents, commits and observations.
...
- [x] Exact call/result and persistence counts recorded above.
...
- Current state: sole call failed closed on one non-canonical timestamp; no
  disposable state remains.
- Next recommended step: freeze an offline prompt-version successor with exact
  UTC seconds/`Z` grammar and `unknown` fallback.
- Blockers: provider returned local minute precision where schema requires UTC.
- Child tasks: none.
