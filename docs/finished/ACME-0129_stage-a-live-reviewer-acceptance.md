...
Superseded at: 2026-08-15

## Supersession

This charter is superseded before its acceptance run. The charter text below is
preserved unedited; nothing in it is corrected retroactively.

The charter's premise was that POC #1 still had to prove live execution could
happen safely without uncontrolled spend. That premise is spent. ACME-0111
through ACME-0122 proved the provider path, and ACME-0121 committed real
source-bound observations from a real call. What the charter still encoded —
six calls total, one shot, never retry, no repair after a consumed call — was a
phase control from a period when the question was whether the architecture
could execute at all.

Keeping it now inverts the experiment. A run bounded to six calls, forbidden to
retry and forbidden to correct code mid-flight does not test the Evidence
Integrity Workbench; it tests an artificially handicapped variant of it. The
first real browser use made that concrete: within one session the product wedged
itself at product revision 2 against engine revision 5, the worker was found
writing observations before the guard that rejects them, and sessions expired
fifteen minutes after process start. None of those are visible to a six-call
one-shot Vitest gate, and all of them are exactly what a real workflow exposes.

The successor charter therefore asks a different question. Not "can we execute
live safely?" but "can the product carry a real evidence workflow end to end?"
Cost stops being governed by an arbitrary call ceiling and starts being
governed by measurement: `acme.model_calls` already records every provider call,
and if a real case needs seven calls to complete, that number is POC evidence
rather than a violation.

- Phase decision: [ADR-0044](../adr/0044-poc1-live-product-acceptance-phase.md)
- Successor correctness task: ACME-0131
- Successor acceptance proposal:
  [poc1-live-product-acceptance.md](../backlog/poc1-live-product-acceptance.md)

No provider call was made under this charter. The seven calls recorded in
`acme.model_calls` were made through the browser outside it.
