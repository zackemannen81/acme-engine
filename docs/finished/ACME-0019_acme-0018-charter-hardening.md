...
- [x] Resolve findings 1–3: ADR role, fingerprint content, effective-policy
      validation.
- [x] Resolve findings 4–5: retrieval evidence and the constant retrieval limit.
- [x] Resolve findings 6–8: digest comparison, replay execution mode, the
      `unavailable` branch and the replay-evidence read shape.
- [x] Resolve findings 9–11: `replayed` semantics, the golden-digest
      verification item and the `AbortSignal` decision.
- [x] Record the reasoning in the parent's `Decisions and Notes` and mark
      maintainer judgment calls.
- [x] Run every frozen verification gate and record exact evidence.
- [x] Restore the hardened parent to `docs/CURRENT_TASK.md` as `Draft`.
- [x] Update `docs/CURRENT_STATUS.md` and add a signed journal entry.
...
- [x] `node tooling/docs/check-docs.mjs` passed for 51 Markdown files, both
      before and after the archive/restore moves. Internal links and fences are
      balanced.
- [x] `git diff --check` passed.
- [x] `git status` shows changes only under `docs/`: `docs/CURRENT_TASK.md`
      modified, `docs/finished/ACME-0019_acme-0018-charter-hardening.md` added,
      `docs/JOURNAL.md` and `docs/CURRENT_STATUS.md` modified. No file under
      `packages/`, `apps/`, `tooling/`, `tests/` or any manifest was touched.
- [x] Each of the eleven findings is traceable to explicit parent charter text
      in In Scope, Out of Scope, Definition of Done, Decisions and Notes or
      Verification.
- [x] The parent's Goal and Primary Deliverable are textually unchanged. Its
      Definition of Done was sharpened in four places — ADR role, `replayed`
      semantics, replay mode/digest comparison and `ModelSelection` conflict
      coverage — and still describes the same Milestone 1 ExecutionEngine
      outcome with the same acceptance proof.
- [x] No check was skipped. No runtime gate applies because this task adds no
      source file; `docs/paused/` is empty apart from its README.

## Documentation Updates

- [x] `docs/CURRENT_TASK.md` (restored hardened ACME-0018 `Draft`)
- [x] `docs/CURRENT_STATUS.md`
- [x] `docs/JOURNAL.md`
- [x] `docs/FILESTRUCTURE.md` — no change required; the archive and paused
      directories are already described generically and no structure changed.
- [x] ADRs — none changed. The execution identity/replay ADR remains ACME-0018
      work and is deliberately not written here.

## Handoff and Follow-ups

- Current state: Complete. ACME-0018 is restored to `docs/CURRENT_TASK.md` as a
  hardened `Draft`, and `docs/paused/` holds no task file.
- Next recommended step: The maintainer confirms the two remaining judgment
  calls in ACME-0018, then freezes it at `Ready`.
- Blockers: None.
- Child tasks: None.
- Resume condition: Not applicable.
- Open questions: Both remaining questions were handed to ACME-0018 rather than
  answered here, because they are maintainer judgment calls rather than
  correctness requirements:
  - the constant for `acme-memory-retrieval-1`, recommended as 50;
  - whether `execute()` exposes `AbortSignal` in Milestone 1, recommended as
    not at all.
