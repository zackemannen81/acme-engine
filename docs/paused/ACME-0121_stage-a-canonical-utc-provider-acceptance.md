...
- Current state: product acceptance committed successfully; disposable state
  remains isolated while the stale test assertion is corrected offline.
- Next recommended step: complete bounded child ACME-0122 without a provider
  call, then restore this parent and finish content-free cleanup/documentation.
- Blockers: live gate asserts a superseded terminal reason code.
- Child tasks: ACME-0122.
- Resume condition: ACME-0122 aligns the assertion with the established worker
  contract and passes offline verification.
- Open questions: none.
