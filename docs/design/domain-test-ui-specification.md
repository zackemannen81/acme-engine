...
1. Pure S8 HTML renderer over `acme-view-measurement/1`.
2. Read-only `/s8` and `/api/measurement` routes over every readable
   workspace run record, optional request-local min/max rate thresholds and an
   optionally selected stored baseline.
3. Separate deterministic/live cards with numerator, sample size, rate,
   threshold outcome and baseline comparison copied from the existing view.
4. Refusal for invalid bounds, unsafe or missing baseline selection and
   unreadable run files that would silently shrink the measurement sample.

**Exit:** a developer can measure recorded deterministic and live series
without mixing them, inventing a score, promoting a baseline or writing any
workspace or canonical artifact.
