...
function Read-OpenAiKeyIfNeeded {
...
$process = $null
$WorkbenchReused = $false

try {
    $existing = Invoke-WebRequest -Uri $HealthUrl -UseBasicParsing -TimeoutSec 2
    if ($existing.StatusCode -eq 200) {
        if ($AutoImport) {
            Write-Ok "Workbench already answers on $WorkbenchUrl; reusing it for AutoImport."
            Write-Host "    Live capabilities will be verified before any import." -ForegroundColor DarkGray
            $WorkbenchReused = $true
        }
        else {
            Write-Host ""
            ...
            Write-Host "Use -AutoImport to reuse it, or stop it first for a fresh launch."
            exit 0
        }
    }
}
...
Write-Host ""
if ($WorkbenchReused) {
    Write-Host "Workbench PID   : existing process (reused)"
    Write-Host "stdout          : existing process"
    Write-Host "stderr          : existing process"
}
else {
    Write-Host "Workbench PID   : $($process.Id)"
