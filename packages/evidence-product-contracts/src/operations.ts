...
import { EvidenceReviewWorkTargetKindSchema } from './review-operation-schemas.js';
import type { EvidenceProductSnapshot } from './schemas.js';

export * from './review-operation-schemas.js';
  ...
  .superRefine((value, context) => {
    ...
    if (new Set(keys).size !== keys.length)
      context.addIssue({
        code: 'custom',
        path: ['targets'],
        message: 'Bulk targets must be unique.',
      });
    if (
...
export const EvidenceCaseSearchQuerySchema = z
  .object({
    ...
    kinds: z
      ...
      .default([]),
    reviewStanding: z
      .enum(['awaiting-review', 'accepted', 'rejected', 'needs-re-review'])
      .nullable()
      .default(null),
    relationKind: z
      .enum([
...
type SearchItem = z.infer<
  typeof EvidenceCaseSearchResultSchema
>['items'][number];
...
export function searchEvidenceCase(
  ...
  const includesKind = (kind: SearchItem['kind']) =>
    wanted.size === 0 || wanted.has(kind);
  ...
  if (includesKind('source'))
    for (const source of snapshot.sources)
      items.push({
        ...
        title: source.title,
        summary: `${source.logicalArtifactId} version ${String(source.versionOrdinal)}`,
  ...
  if (includesKind('observation'))
    for (const item of snapshot.observations) {
      ...
      items.push({
        ...
        title: actor || 'Source observation',
        summary: `${item.exactQuote} ${actor}`.trim(),
        artifactVersionId: item.artifactVersionId,
        locatorId: item.locator.locatorId,
        reviewStanding: standing(snapshot, 'observation', item.observationId),
        assigneePrincipalRef: assignmentFor(
          snapshot,
          'observation',
          item.observationId,
        ),
  ...
  if (includesKind('relation'))
    for (const item of snapshot.relations) {
      if (
        query.relationKind !== null &&
        item.relationKind !== query.relationKind
      )
        continue;
      items.push({
        kind: 'relation',
        id: item.relationId,
        title: item.relationKind,
        summary: `${item.comparableScope.subject} ${item.comparableScope.aspect} ${item.rationale}`,
        artifactVersionId: null,
        locatorId: null,
        reviewStanding: standing(snapshot, 'relation', item.relationId),
        assigneePrincipalRef: assignmentFor(
          snapshot,
          'relation',
          item.relationId,
  ...
  if (includesKind('open-question'))
    for (const item of snapshot.openQuestions)
      items.push({
        kind: 'open-question',
        id: item.openQuestionId,
        title: item.questionCode,
        summary: item.questionText,
        artifactVersionId: null,
        locatorId: null,
        reviewStanding: null,
        assigneePrincipalRef: null,
  ...
  if (includesKind('assessment'))
    for (const item of snapshot.assessments)
      items.push({
        kind: 'assessment',
        id: item.assessmentVersionId,
        title: `Assessment ${String(item.sequence)}`,
        summary: item.claims.map((claim) => claim.text).join(' '),
        artifactVersionId: null,
        locatorId: null,
  ...
  const filtered = items
    ...
    .filter(
      (item) =>
        query.reviewStanding === null ||
        item.reviewStanding === query.reviewStanding,
    )
    .filter(
      (item) =>
        query.assigneePrincipalRef === null ||
        item.assigneePrincipalRef === query.assigneePrincipalRef,
    )
    .sort((left, right) =>
      `${left.kind}\u0000${left.title}\u0000${left.id}`.localeCompare(
        `${right.kind}\u0000${right.title}\u0000${right.id}`,
      ),
    );
  const offset =
    query.cursor === null ? 0 : Number(query.cursor.slice('offset:'.length));
  ...
  return EvidenceCaseSearchResultSchema.parse({
    ...
    nextCursor:
      nextOffset < filtered.length ? `offset:${String(nextOffset)}` : null,
    items: page,
  });
}
...
export type EvidenceReviewAssignmentCommand = z.infer<
  typeof EvidenceReviewAssignmentCommandSchema
>;
export type EvidenceReviewCommentCommand = z.infer<
  typeof EvidenceReviewCommentCommandSchema
>;
export type EvidenceBulkReviewCommand = z.infer<
  typeof EvidenceBulkReviewCommandSchema
>;
export type EvidenceCaseSearchQuery = z.infer<
  typeof EvidenceCaseSearchQuerySchema
>;
export type EvidenceCaseSearchResult = z.infer<
  typeof EvidenceCaseSearchResultSchema
>;
