...
function selectionKey(selection: ModelSelection): string {
  return [selection.profile, selection.providerHint ?? '', selection.modelHint ?? ''].join('\0');
}
...
function classifyStatus(status: number, body: string): never {
  ...
  if (status === 400 || status === 404 || status === 422) {
    fail('INVALID_REQUEST', 'The provider rejected the request.', false, details);
  }
  if (status === 401 || status === 403) {
    fail('MODEL_AUTH', 'The provider rejected the credentials.', false, details);
  }
  if (status === 408) fail('TIMEOUT', 'The provider reported a request timeout.', true, details);
  if (status === 429) fail('MODEL_RATE_LIMIT', 'The provider rate limited the call.', true, details);
  fail('MODEL_UNAVAILABLE', 'The provider was unavailable.', true, details);
}

function classifyNoResponse(result: Extract<ProviderTransportResult, { kind: 'no-response' }>): never {
  const code: AcmeErrorCode = result.reason === 'timeout' ? 'TIMEOUT' : 'MODEL_UNAVAILABLE';
  const details: JsonValue = {
    reason: result.reason,
    delivery: result.delivery,
    ...(result.message === undefined ? {} : { transportMessage: result.message }),
  };
  if (result.reason === 'aborted' && result.delivery === 'not-sent') {
    fail('CANCELLED', 'The call was cancelled before it was sent.', false, details);
  }
  if (result.delivery === 'not-sent') fail(code, 'The request never reached the provider.', true, details);
  failAmbiguous(code, 'The provider may have executed this call; no response was received.', details);
}
function requireControl(
  ...
  if (supplied && !profile.controls?.[key]) {
    fail('UNSUPPORTED_CAPABILITY', `The selected model cannot honor ${key}.`, false, {
...
function validateControls(request: ModelRequest, profile: ChatCompletionsModelProfile): void {
  requireControl(profile, 'temperature', request.temperature !== undefined);
  requireControl(profile, 'topP', request.topP !== undefined);
  requireControl(profile, 'maxOutputTokens', request.maxOutputTokens !== undefined);
  requireControl(profile, 'stop', request.stop !== undefined);
  requireControl(profile, 'reasoningBudget', request.reasoningBudget !== undefined);
  requireControl(profile, 'enableThinking', request.enableThinking !== undefined);
  requireControl(profile, 'reasoningEffort', request.reasoningEffort !== undefined);
...
function textContent(parts: ModelRequest['messages'][number]['content'], label: string): string {
  let text = '';
  for (const part of parts) {
    if (part.type !== 'text') {
      fail('INVALID_REQUEST', `${label} contains a content part this provider surface cannot map.`, false, {
        partType: part.type,
      });
    }
    text += part.text;
...
function wireMessages(request: ModelRequest): readonly JsonObject[] {
  ...
  request.messages.forEach((message, messageIndex) => {
    if (message.role === 'tool') {
      for (const part of message.content) {
        if (part.type !== 'tool-result') {
          fail('INVALID_REQUEST', 'Tool messages may only contain tool-result parts.', false, {
            messageIndex,
            partType: part.type,
          });
        }
    ...
    if (unsupported !== undefined) {
      fail('INVALID_REQUEST', 'Assistant message contains an unsupported content part.', false, {
        messageIndex,
    ...
    result.push({
      role: 'assistant',
      content: texts.length === 0 ? null : texts.map((part) => part.type === 'text' ? part.text : '').join(''),
      ...(calls.length === 0 ? {} : {
        tool_calls: calls.map((part) => part.type === 'tool-call' ? {
          id: part.toolCallId,
          type: 'function',
...
function buildBody(
  ...
  if (request.output.mode !== 'text') {
    fail('UNSUPPORTED_CAPABILITY', 'This Chat Completions profile does not expose ACME structured output.', false, {
      provider: profile.provider,
      model: profile.model,
    });
  }
  ...
  if (request.reasoningBudget !== undefined) body.reasoning_budget = request.reasoningBudget;
  ...
  if (request.enableThinking !== undefined) {
    ...
    if (mode === 'enable_thinking') body.chat_template_kwargs = { enable_thinking: request.enableThinking };
    if (mode === 'thinking') body.chat_template_kwargs = { thinking: request.enableThinking };
  }
  return body;
}
...
function parseToolArguments(raw: string, toolCallId: string): JsonValue {
  ...
  if (parsed === undefined) {
    fail('MODEL_INVALID_RESPONSE', 'A tool call had malformed JSON arguments.', false, { toolCallId });
  }
...
function normalizedToolCalls(value: unknown): readonly NormalizedToolCall[] {
  ...
  if (!Array.isArray(value)) fail('MODEL_INVALID_RESPONSE', 'Provider tool_calls must be an array.', false);
  ...
  return value.map((item, index) => {
    ...
    if (!id || !name || args === undefined || call?.type !== 'function' || seen.has(id)) {
      fail('MODEL_INVALID_RESPONSE', 'Provider returned an invalid or duplicate tool call.', false, { index });
    }
    seen.add(id);
    return { toolCallId: id, name, arguments: parseToolArguments(args, id) };
  });
}
function normalizedFinishReason(raw: unknown, calls: readonly NormalizedToolCall[]): NormalizedModelResponse['finishReason'] {
  if (calls.length > 0 || raw === 'tool_calls' || raw === 'function_call') return 'tool';
...
function normalizeBuffered(
  ...
  const parsed = record(safeJson(raw));
  if (parsed === undefined) {
    fail('MODEL_INVALID_RESPONSE', 'The provider returned invalid JSON.', false);
  }
  const choices = parsed.choices;
  if (!Array.isArray(choices) || choices.length === 0) {
    fail('MODEL_INVALID_RESPONSE', 'The provider returned no chat choice.', false);
  }
  const choice = record(choices[0]);
  const message = record(choice?.message);
  if (choice === undefined || message === undefined) {
    fail('MODEL_INVALID_RESPONSE', 'The provider returned an invalid chat choice.', false);
  }
  const text = message.content === null ? '' : optionalString(message.content);
  if (text === undefined) {
    fail('MODEL_INVALID_RESPONSE', 'The provider returned invalid assistant content.', false);
  }
  ...
  const response: NormalizedModelResponse = {
    ...
    ...(optionalString(parsed.id) === undefined ? {} : { providerResponseId: optionalString(parsed.id) }),
    receivedAt,
...
export class ChatCompletionsGateway implements ModelGateway {
  ...
  constructor(options: ChatCompletionsGatewayOptions) {
    ...
    for (const input of options.profiles) {
      ...
      if (input.provider.trim().length === 0 || input.model.trim().length === 0 || input.endpoint.trim().length === 0) {
        throw new Error('Chat Completions profiles require provider, model and endpoint.');
      }
      const key = selectionKey(selection);
      if (this.#profiles.has(key)) throw new Error(`Duplicate Chat Completions profile: ${key}`);
      this.#profiles.set(key, Object.freeze({ ...input, selection, capabilities }));
    }
  }
  ...
  async generate(request: ModelRequest, context: GatewayCallContext): Promise<NormalizedModelResponse> {
    ...
    assertRequiredModelCapabilities(profile.capabilities, validatedContext.requiredCapabilities);
    ...
    const result = await this.#transport.send(this.#transportRequest(
      profile,
      buildBody(validatedRequest, profile, false),
      validatedContext,
    ));
    if (result.kind === 'no-response') classifyNoResponse(result);
    if (result.status < 200 || result.status >= 300) classifyStatus(result.status, result.body);
    return normalizeBuffered(result.body, profile, this.#now());
  }
  async *stream(request: ModelRequest, context: GatewayCallContext): AsyncIterable<ModelStreamEvent> {
    const validatedContext = validateGatewayCallContext(context);
    const profile = this.#profile(validatedContext.selection);
    const validatedRequest = validateModelRequest(request);
    assertRequiredModelCapabilities(profile.capabilities, validatedContext.requiredCapabilities);
    ...
    if (this.#transport.stream === undefined) {
      fail('UNSUPPORTED_CAPABILITY', 'The configured transport cannot stream.', false);
    }
    ...
    const parseData = (data: string): ModelStreamEvent[] => {
      ...
      if (parsed === undefined) {
        fail('MODEL_INVALID_RESPONSE', 'The provider emitted malformed streaming JSON.', false);
      }
      ...
      const choice = record(choices[0]);
      if (choice === undefined) {
        fail('MODEL_INVALID_RESPONSE', 'The provider emitted an invalid streaming choice.', false);
      }
      ...
      if (reasoning !== undefined && reasoning.length > 0) {
        emitted.push({ type: 'reasoning-delta', sequence: sequence++, text: reasoning });
      }
      const content = optionalString(delta.content);
      if (content !== undefined && content.length > 0) {
        text += content;
        emitted.push({ type: 'content-delta', sequence: sequence++, text: content });
      }
      if (delta.tool_calls !== undefined) {
        if (!Array.isArray(delta.tool_calls)) {
          fail('MODEL_INVALID_RESPONSE', 'Provider tool-call delta must be an array.', false);
        }
        for (const rawCall of delta.tool_calls) {
          const call = record(rawCall);
          if (call === undefined || !Number.isSafeInteger(call.index) || Number(call.index) < 0) {
            fail('MODEL_INVALID_RESPONSE', 'Provider tool-call delta has an invalid index.', false);
          }
          const index = Number(call.index);
          const state = toolState.get(index) ?? { id: '', name: '', arguments: '' };
          ...
          if ((idPart?.length ?? 0) > 0 || (namePart?.length ?? 0) > 0 || (argsPart?.length ?? 0) > 0) {
            emitted.push({
              type: 'tool-call-delta',
              sequence: sequence++,
              index,
              ...((idPart?.length ?? 0) === 0 ? {} : { toolCallId: idPart }),
              ...((namePart?.length ?? 0) === 0 ? {} : { name: namePart }),
              ...((argsPart?.length ?? 0) === 0 ? {} : { argumentsDelta: argsPart }),
            });
          }
        }
      }
    ...
    const transportStream = this.#transport.stream(this.#transportRequest(
      profile,
      buildBody(validatedRequest, profile, true),
      validatedContext,
    ));
    for await (const event of transportStream) {
      ...
      if (event.kind === 'no-response') {
        if (responseStarted && responseStatus !== undefined && responseStatus >= 200 && responseStatus < 300) {
          fail('MODEL_INVALID_RESPONSE', 'The provider stream ended after HTTP success without a complete terminal response.', false, {
    ...
    if (!responseStarted || responseStatus === undefined) {
      failAmbiguous('MODEL_UNAVAILABLE', 'The provider stream ended without an HTTP status.', {
        delivery: 'unknown',
      });
    }
    if (responseStatus < 200 || responseStatus >= 300) classifyStatus(responseStatus, errorBody);
    for (const data of decoder.finish()) {
      for (const output of parseData(data)) yield output;
    }
    if (!sawDone && finishReason === undefined) {
      fail('MODEL_INVALID_RESPONSE', 'The provider stream was truncated before a terminal finish reason.', false);
    }

    const toolCalls: NormalizedToolCall[] = [];
    for (const [index, state] of [...toolState.entries()].sort(([a], [b]) => a - b)) {
      if (state.id.length === 0 || state.name.length === 0) {
        fail('MODEL_INVALID_RESPONSE', 'A streamed tool call was structurally incomplete.', false, { index });
      }
      toolCalls.push({
        toolCallId: state.id,
        name: state.name,
  ...
  #profile(selection: ModelSelection): ChatCompletionsModelProfile {
    ...
    if (profile === undefined) {
      fail('INVALID_REQUEST', 'No configured Chat Completions profile matches the selection.', false, {
