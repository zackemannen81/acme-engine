...
/**
 * These journeys drive a real HTTP server, a filesystem repository and a full
 * execution through the engine. Isolated they take a couple of seconds; run
 * alongside the rest of the suite on a loaded machine they exceed vitest's
 * 5s default and fail as timeouts rather than as anything real.
 *
 * The timeout is declared per test rather than raised globally, so a genuinely
 * hung test elsewhere still fails fast.
 */
const BLACK_BOX_TIMEOUT_MS = 60_000;

describe('local Evidence workbench', () => {
  it(
    'assigns, comments, bulk reviews and searches only inside the selected case',
    async () => {
      ...
      try {
        ...
        const overview = (await (
          ...
          counts: { sources: number; pendingObservations: number };
        };
    ...
    BLACK_BOX_TIMEOUT_MS,
  );

  it(
    'imports bounded synthetic text and applies immutable redaction through case-first routes',
    async () => {
      const directory = await mkdtemp(
  ...
  it(
    'runs source import, polling, primary views and review with technical audit disabled',
    async () => {
      const directory = await mkdtemp(
  ...
  it(
    'compares corrected and later accounts with every prior source version navigable',
    async () => {
      const directory = await mkdtemp(
  ...
  it(
    'completes assessment review, late-evidence attention, re-review and deterministic export',
    async () => {
      const directory = await mkdtemp(
      ...
      const local = await createLocalEvidenceWorkbench({
        ...
        reviewIds: {
          next: () =>
            `assessment-review-${String(++reviewId).padStart(4, '0')}`,
        },
      });
      const address = await listenEvidenceWorkbenchApi(local.server, {
        port: 0,
