 ...
 * The Fetch host deliberately cancels its reader when the bounded body limit is
 * exceeded. Destroying the IncomingMessage at that point also destroys the
 * shared HTTP socket and prevents the 413 response from reaching the client.
 * Instead, cancellation detaches the consumer and drains the remaining bytes.
 */
function requestBody(
  ...
  return new ReadableStream<Uint8Array>({
    ...
    cancel() {
      ...
      active = false;
      cleanup();
      // Consume and discard the rest of the request without killing the socket.
      request.resume();
    },
  });
}
...
async function releaseUnconsumedRequestBody(
  ...
  try {
    ...
    // A body that was already consumed/locked no longer needs the private stream
    // cancellation path. Resume the Node request as the safe transport fallback.
...
function transportFailure(target: ServerResponse): void {
  ...
  const body = JSON.stringify({
    protocolVersion: 'aal-acme-runtime-error/1',
...
/**
 * Adapt the verified Fetch-compatible runtime host to Node's built-in HTTP
 * server. This layer owns sockets only; runtime auth/protocol/body policy stays
 * inside the Fetch host.
 */
...
export function createBearerAuthorizer(token: string): AcmeRuntimeAuthorizer {
  const expected = Buffer.from(token, 'utf8');
  if (expected.byteLength < 32 || expected.byteLength > 4096) {
    throw new Error(
      'Runtime bearer token must contain 32 through 4096 UTF-8 bytes.',
    );
  }
